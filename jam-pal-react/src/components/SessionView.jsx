...
function BassIcon() {
  ...
  return (
    <svg viewBox="0 0 612 612" fill="currentColor" stroke="currentColor" strokeWidth="14" xmlns="http://www.w3.org/2000/svg">
...
function KeysIcon() {
  ...
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round">
      <rect x="2" y="4" width="20" height="16" rx="1.5" strokeWidth="1.0" />
...
function SessionView({
  ...
  genre = 'rock', style = 'supportive', timeSig = '4/4', instrument = 'guitar',
  jamMode = null,
  timing = null,
  getFrequencyData,
  ...
  }, [onToggleMic, onToggleRecording, onToggleLoop, onToggleMetronome, onToggleLock, onEndSession, showShortcuts]);
    ...
    <div className={styles.console}>
        ...
        <div className={styles.ringWrap}>
          ...
          <div className={styles.ringContent}>
            ...
            <div className={styles.chips}>
              ...
              {instrument !== 'guitar' && <span className={styles.chip}>on · {instrument}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`${styles.mixer} ${styles.mixerPanel}`}>
        ...
        <div className={styles.channels}>
          {[
            { id: 'drums',  label: 'Drums',  value: drumVolume,   onChange: onDrumVolume },
            { id: 'keys',   label: 'Keys',    value: keysVolume,   onChange: onKeysVolume },
            { id: 'guitar', label: 'Guitar',  value: guitarVolume, onChange: onGuitarVolume },
            { id: 'bass',   label: 'Bass',    value: bassVolume,   onChange: onBassVolume },
          ].map((channel) => {
            const isMuted = mutedChannels.has(channel.id);
            const isSoloed = soloedChannels.has(channel.id);
            const fillPct = Math.min(100, (channel.value / 1.5) * 100);
            return (
              <div key={channel.id} className={`${styles.channel} ${styles[channel.id]}`}>
                <div className={`${styles.icon} ${styles[channel.id]}`}>
                  {channel.id === 'drums' && <DrumIcon />}
                  {channel.id === 'keys' && <KeysIcon />}
                  {channel.id === 'bass' && <BassIcon />}
                  {channel.id === 'guitar' && <GuitarIcon />}
                </div>

                <div className={`${styles.label} ${styles[channel.id]}`}>
                  {channel.label}
                </div>
                ...
                <div className={styles.faderTrack}>
                  <input
                    ...
                    className={`${styles.verticalSlider} ${styles[channel.id]}`}
                    style={{ '--fill': `${fillPct}%` }}
                  />
                </div>
                ...
                <div className={styles.controls}>
                  <button
                    className={`${styles.smButton} ${isSoloed ? styles.smButtonSolo : ''}`}
                    onClick={() => onToggleSolo?.(channel.id)}
                    aria-pressed={isSoloed}
                    aria-label={`Solo ${channel.label}`}
                    title={`Solo ${channel.label}`}
                  >S</button>
                  <button
                    className={`${styles.smButton} ${isMuted ? styles.smButtonMute : ''}`}
                    onClick={() => onToggleMute?.(channel.id)}
                    aria-pressed={isMuted}
                    aria-label={`Mute ${channel.label}`}
                    title={`Mute ${channel.label}`}
                  >M</button>
                </div>
              </div>
            );
